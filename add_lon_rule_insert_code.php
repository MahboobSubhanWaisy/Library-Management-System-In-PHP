<?php 
    ...
    if(isset($_POST['submit-book-btn'])){
        ...
        $check_copy = mysqli_query($con, 'SELECT copy_id FROM rule_book WHERE r_id = "'.$bb_id.'"');
        $row = mysqli_fetch_array($check_copy);
        $old_copy_number = $row[0];
        if($old_copy_number > 0){
            $new_copy = ($old_copy_number - 1);
            $update_copy = mysqli_query($con, "UPDATE rule_book SET `copy_id` = ".$new_copy." WHERE r_id = ".$bb_id."");
            if($update_copy){
